...
generation_PMMR <- function(seed, n.sample=400, setting="linear_1"){
  ...
  # n.train <- n.sample*(2/3); n.test = n.sample*(1/3)
  ...
  for (i in 1:n.sample){
    ...
    #sim.data[i,] <- c(y,zwu[2],zwu[3],zwu[1],x)
  ...
  ha <- c(ha.train, ha.test)
  
  setup <- paste0(setting, "_size", n.sample)
  write.csv(sim.data, paste0(file.path(root.dir, setup),"/", "sim_", seed, ".csv"), row.names = FALSE)
  write.csv(treatment, paste0(file.path(root.dir, setup),"/", "do_A_", seed, ".csv"), row.names = FALSE)
  write.csv(ha, paste0(file.path(root.dir, setup),"/", "EY_do_A_", seed, ".csv"), row.names = FALSE)
  
  print(c(paste0("Simulation for setup ", setup, " with seed ", seed, " is completed")))
}