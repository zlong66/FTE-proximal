...
def get_causal_effect_crossfit(do_A, WX_marginal, X, Y, W_, wx_dim, ker1, ker2, poly, params_l, sigma_a, offset=0, k=3):
    ...
    ate_est_crossfit = np.array(ate_est)
    ate_est_ave = np.mean(ate_est_crossfit, axis=0)
    
    
    print('Estimated ATE: ', ate_est_ave)
...
def run_pmmr_rkhs(seed, al_diff_search_range, bl_search_range, train_sizes, data_seeds, sname, args):
    ...
    for train_size in train_sizes:
        ...
        for data_seed in data_seeds:
            SAVE_PATH = os.path.join(ROOT_PATH, "PMMR results/", sname)
